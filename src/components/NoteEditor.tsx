...
/**
 * @deprecated This component is being migrated to NoteEditorRefactored.
 * Please use /src/components/NoteEditorRefactored/NoteEditorRefactored.tsx for new development.
 ...
 */
function _getXFromBeat(beat: number, beatWidth: number = BEAT_WIDTH): number {
...
export function NoteEditor({
  ...
  systemCount,
  totalMeasures,
  onSystemCountChange: _onSystemCountChange,
  onDuplicateNote,
  ...
  // Learn mode props
  learnMode: _learnMode = false,
  maxVisibleMeasures: _maxVisibleMeasures,
  highlightPitchLine: _highlightPitchLine,
  highlightBeatRange: _highlightBeatRange,
  onNoteInteraction: _onNoteInteraction,
}: NoteEditorProps) {
  ...
  // Dynamic system height: 5 lines needs more vertical space to prevent clef overlap
  const effectiveSystemHeight =
    staffLines === 5
      ? SYSTEM_HEIGHT + 60 // 260px for 5 lines
      : staffLines === 4
        ? SYSTEM_HEIGHT + 20 // 220px for 4 lines
        : SYSTEM_HEIGHT; // 200px for 3 lines

  // Local override of getStaffCenterY to use dynamic system height
  const getStaffCenterY = useCallback(
    (systemIndex: number): number => {
      return (
        SYSTEM_TOP_MARGIN +
        systemIndex * effectiveSystemHeight +
        STAFF_CENTER_OFFSET
      );
    },
    [effectiveSystemHeight],
  );
      ...
      <g key={`system-${systemIndex}`}>
        ...
        {repeatStart &&
          ...
          (() => {
            ...
            const _showTextInMeasure = hoveredRepeatMeasure
        ...
        */}
        <g clipPath={`url(#system-clip-${systemIndex})`}>
          <g
            transform={`translate(-1, ${staffCenterY - 98}) scale(5.2)`}
            style={{ pointerEvents: "none" }}
          >
            <path
          ...
          for (let i = 0; i < groupNotes.length; i++) {
            ...
            if (note.duration === 0.25) {
              ...
              if (nextIs16th && !prevIs16th) {
                ...
                const _beamY = getBeamYAtX(stemX) + secondaryBeamOffset;
